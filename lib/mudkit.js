...
function MUDKit (port, host, callback, terminal) {
    ...
    this.mud.setKeepAlive(true, 7 * 60 * 1000);
    keepAlive = setTimeout(function () {
        console.log('keep alive');
        mudkit.sendCommand(NOP);
    }, 3 * 60 * 1000);
    ...
    this.mud.on('error', function (err) {
        console.log(err);
        // util.debug(err);
    });
...
MUDKit.prototype.sendCommand = function (command) {
    ...
    this.mud.write(new Buffer(bytes));
};
...
MUDKit.prototype.processChunk = function (chunk, encoding) {
    ...
    for (i = 0, l = chunk.length; i < l; i++) {
        ...
        if (CR === chunk[i] && LF === chunk[i+1]) {
            ...
            process255();
            last = i + 1;
        } else if (38 === chunk[i] || 34 === chunk[i] || 39 === chunk[i] || 60 === chunk[i] || 62 === chunk[i]) {
            output += sliceChunk();
            escapeHTML(chunk[i]);
            last = i + 1;
        }
